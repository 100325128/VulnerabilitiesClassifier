...
class VulnerabilitiesHandler(ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if (name == CONST_CVE_ID or name == CONST_SUMMARY or name == CONST_PRODUCT) :
    ...
    def endElement(self,name):
        ...
        if (name == CONST_ENTRY) :
            if(not self.fileManager.isCandidate(self.cve_id)) :            
                self.fileManager.writeNoClassify(self.cve_id, self.cve_vulnerability, self.vulnerable_nc_list)
                self.fileManager.writeApplication(self.cve_id, self.cve_vulnerability, self.vulnerable_ap_list)
                self.fileManager.writeDesktop(self.cve_id, self.cve_vulnerability, self.vulnerable_desktop_list)
                self.fileManager.writeMobile(self.cve_id, self.cve_vulnerability, self.vulnerable_mobile_list)
                if (len(self.vulnerable_nc_list) == 0 and len(self.vulnerable_ap_list) == 0 and
                    ...
                    self.fileManager.writeDesktopDefault(self.cve_id, self.cve_vulnerability)
